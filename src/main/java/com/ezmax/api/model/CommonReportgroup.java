...
public class CommonReportgroup {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CommonReportgroup
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CommonReportgroup.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CommonReportgroup.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CommonReportgroup` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayaObjReport.size(); i++) {
        CommonReport.validateJsonObject(jsonArrayaObjReport.get(i).getAsJsonObject());
      };
      // ensure the json data is an array
      ...
      for (int i = 0; i < jsonArrayaObjReportcellstyleCustom.size(); i++) {
        CommonReportcellstyle.validateJsonObject(jsonArrayaObjReportcellstyleCustom.get(i).getAsJsonObject());
      };
  }
