  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustomEzsignfoldertransmissionResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in CustomEzsignfoldertransmissionResponse is not found in the empty JSON string", CustomEzsignfoldertransmissionResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CustomEzsignfoldertransmissionResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `CustomEzsignfoldertransmissionResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("a_objEzsignfoldertransmissionSigner").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `a_objEzsignfoldertransmissionSigner` to be an array in the JSON string but got `%s`", jsonObj.get("a_objEzsignfoldertransmissionSigner").toString()));
      }
