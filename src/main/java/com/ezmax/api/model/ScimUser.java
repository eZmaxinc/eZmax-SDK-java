...
public class ScimUser {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ScimUser
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ScimUser.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ScimUser.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScimUser` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("emails") != null && !jsonObj.get("emails").isJsonNull()) {
        ...
        if (jsonArrayemails != null) {
          ...
          for (int i = 0; i < jsonArrayemails.size(); i++) {
            ScimEmail.validateJsonObject(jsonArrayemails.get(i).getAsJsonObject());
          };
        }
      }
  }
