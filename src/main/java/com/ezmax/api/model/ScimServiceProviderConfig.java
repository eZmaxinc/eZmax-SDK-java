...
public class ScimServiceProviderConfig {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ScimServiceProviderConfig
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ScimServiceProviderConfig.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ScimServiceProviderConfig.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScimServiceProviderConfig` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayauthenticationSchemes.size(); i++) {
        ScimAuthenticationScheme.validateJsonObject(jsonArrayauthenticationSchemes.get(i).getAsJsonObject());
      };
      // validate the required field `bulk`
      ScimServiceProviderConfigBulk.validateJsonObject(jsonObj.getAsJsonObject("bulk"));
      // validate the required field `changePassword`
      ScimServiceProviderConfigChangePassword.validateJsonObject(jsonObj.getAsJsonObject("changePassword"));
      ...
      ScimServiceProviderConfigEtag.validateJsonObject(jsonObj.getAsJsonObject("etag"));
      // validate the required field `filter`
      ScimServiceProviderConfigFilter.validateJsonObject(jsonObj.getAsJsonObject("filter"));
      // validate the required field `patch`
      ScimServiceProviderConfigPatch.validateJsonObject(jsonObj.getAsJsonObject("patch"));
      // validate the required field `sort`
      ScimServiceProviderConfigSort.validateJsonObject(jsonObj.getAsJsonObject("sort"));
  }
