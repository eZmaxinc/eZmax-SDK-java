  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ActivesessionResponseCompoundUser.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ActivesessionResponseCompoundUser is not found in the empty JSON string", ActivesessionResponseCompoundUser.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ActivesessionResponseCompoundUser.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ActivesessionResponseCompoundUser` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("sAvatarUrl") != null && !jsonObj.get("sAvatarUrl").isJsonNull()) && !jsonObj.get("sAvatarUrl").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `sAvatarUrl` to be a primitive type in the JSON string but got `%s`", jsonObj.get("sAvatarUrl").toString()));
      }
