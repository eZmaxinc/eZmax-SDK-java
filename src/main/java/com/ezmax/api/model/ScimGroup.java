...
public class ScimGroup {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ScimGroup
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ScimGroup.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ScimGroup.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScimGroup` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("members") != null && !jsonObj.get("members").isJsonNull()) {
        ...
        if (jsonArraymembers != null) {
          ...
          for (int i = 0; i < jsonArraymembers.size(); i++) {
            ScimGroupMember.validateJsonObject(jsonArraymembers.get(i).getAsJsonObject());
          };
        }
      }
  }
