...
public class WebsocketResponseGetWebsocketIDV1MPayload {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("sWebsocketID"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("sWebsocketID"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!WebsocketResponseGetWebsocketIDV1MPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in WebsocketResponseGetWebsocketIDV1MPayload is not found in the empty JSON string", WebsocketResponseGetWebsocketIDV1MPayload.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WebsocketResponseGetWebsocketIDV1MPayload.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `WebsocketResponseGetWebsocketIDV1MPayload` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("sWebsocketID").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `sWebsocketID` to be a primitive type in the JSON string but got `%s`", jsonObj.get("sWebsocketID").toString()));
      }
  }
