  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsignfolderBatchDownloadV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsignfolderBatchDownloadV1Request is not found in the empty JSON string", EzsignfolderBatchDownloadV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsignfolderBatchDownloadV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsignfolderBatchDownloadV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("a_eDocumentType") == null) {
        ...
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `a_eDocumentType` to be an array in the JSON string but got `%s`", jsonObj.get("a_eDocumentType").toString()));
      }
  }
