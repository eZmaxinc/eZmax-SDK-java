  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsigntemplatedocumentExtractTextV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsigntemplatedocumentExtractTextV1Request is not found in the empty JSON string", EzsigntemplatedocumentExtractTextV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsigntemplatedocumentExtractTextV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsigntemplatedocumentExtractTextV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("eSection") != null && !jsonObj.get("eSection").isJsonNull()) && !jsonObj.get("eSection").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `eSection` to be a primitive type in the JSON string but got `%s`", jsonObj.get("eSection").toString()));
      }
