  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsigntemplatedocumentGetWordsPositionsV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsigntemplatedocumentGetWordsPositionsV1Request is not found in the empty JSON string", EzsigntemplatedocumentGetWordsPositionsV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsigntemplatedocumentGetWordsPositionsV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsigntemplatedocumentGetWordsPositionsV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("eGet").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `eGet` to be a primitive type in the JSON string but got `%s`", jsonObj.get("eGet").toString()));
      }
      ...
      if (jsonObj.get("a_sWord") != null && !jsonObj.get("a_sWord").isJsonNull() && !jsonObj.get("a_sWord").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `a_sWord` to be an array in the JSON string but got `%s`", jsonObj.get("a_sWord").toString()));
      }
  }
