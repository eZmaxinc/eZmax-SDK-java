  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustomWebhooklogResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in CustomWebhooklogResponse is not found in the empty JSON string", CustomWebhooklogResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CustomWebhooklogResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `CustomWebhooklogResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("dtWebhooklogDate").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `dtWebhooklogDate` to be a primitive type in the JSON string but got `%s`", jsonObj.get("dtWebhooklogDate").toString()));
      }
      if (!jsonObj.get("tWebhooklogJson").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `tWebhooklogJson` to be a primitive type in the JSON string but got `%s`", jsonObj.get("tWebhooklogJson").toString()));
      }
  }
