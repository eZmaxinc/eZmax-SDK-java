...
public class UsergroupmembershipRequest {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UsergroupmembershipRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in UsergroupmembershipRequest is not found in the empty JSON string", UsergroupmembershipRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UsergroupmembershipRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `UsergroupmembershipRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : UsergroupmembershipRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UsergroupmembershipRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UsergroupmembershipRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UsergroupmembershipRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UsergroupmembershipRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<UsergroupmembershipRequest>() {
           @Override
           public void write(JsonWriter out, UsergroupmembershipRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UsergroupmembershipRequest read(JsonReader in) throws IOException {
