...
public class EzsignbulksenddocumentmappingCreateObjectV1Request {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("a_objEzsignbulksenddocumentmapping"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("a_objEzsignbulksenddocumentmapping"));
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to EzsignbulksenddocumentmappingCreateObjectV1Request
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsignbulksenddocumentmappingCreateObjectV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsignbulksenddocumentmappingCreateObjectV1Request is not found in the empty JSON string", EzsignbulksenddocumentmappingCreateObjectV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsignbulksenddocumentmappingCreateObjectV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsignbulksenddocumentmappingCreateObjectV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayaObjEzsignbulksenddocumentmapping.size(); i++) {
        EzsignbulksenddocumentmappingRequestCompound.validateJsonElement(jsonArrayaObjEzsignbulksenddocumentmapping.get(i));
      };
  }
