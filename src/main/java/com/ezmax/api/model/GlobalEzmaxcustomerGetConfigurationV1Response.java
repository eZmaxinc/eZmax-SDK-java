...
public class GlobalEzmaxcustomerGetConfigurationV1Response {
  ...
  * @throws IOException if the JSON Object is invalid with respect to GlobalEzmaxcustomerGetConfigurationV1Response
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!GlobalEzmaxcustomerGetConfigurationV1Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!GlobalEzmaxcustomerGetConfigurationV1Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GlobalEzmaxcustomerGetConfigurationV1Response` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
