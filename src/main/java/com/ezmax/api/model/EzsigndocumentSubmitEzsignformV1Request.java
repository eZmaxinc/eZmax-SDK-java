...
public class EzsigndocumentSubmitEzsignformV1Request {
  ...
  * @throws IOException if the JSON Object is invalid with respect to EzsigndocumentSubmitEzsignformV1Request
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!EzsigndocumentSubmitEzsignformV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!EzsigndocumentSubmitEzsignformV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EzsigndocumentSubmitEzsignformV1Request` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayaObjEzsignformfieldgroup.size(); i++) {
        CustomEzsignformfieldgroupRequest.validateJsonObject(jsonArrayaObjEzsignformfieldgroup.get(i).getAsJsonObject());
      };
  }
