...
public class CommonReportcell {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("iReportcellColumnspan", "iReportcellRowspan", "sReportcellContent", "iReportcellColumn"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("iReportcellColumnspan", "iReportcellRowspan", "sReportcellContent", "iReportcellColumn"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CommonReportcell.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in CommonReportcell is not found in the empty JSON string", CommonReportcell.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CommonReportcell.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `CommonReportcell` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CommonReportcell.openapiRequiredFields) {
      ...
      if (!jsonObj.get("sReportcellContent").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `sReportcellContent` to be a primitive type in the JSON string but got `%s`", jsonObj.get("sReportcellContent").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CommonReportcell.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CommonReportcell' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CommonReportcell> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CommonReportcell.class));

       return (TypeAdapter<T>) new TypeAdapter<CommonReportcell>() {
           @Override
           public void write(JsonWriter out, CommonReportcell value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CommonReportcell read(JsonReader in) throws IOException {
