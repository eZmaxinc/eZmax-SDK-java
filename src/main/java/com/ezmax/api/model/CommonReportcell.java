...
public class CommonReportcell {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CommonReportcell
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CommonReportcell.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CommonReportcell is not found in the empty JSON string", CommonReportcell.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CommonReportcell.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CommonReportcell` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
