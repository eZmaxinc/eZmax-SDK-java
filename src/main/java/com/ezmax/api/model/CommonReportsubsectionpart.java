...
public class CommonReportsubsectionpart {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CommonReportsubsectionpart
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CommonReportsubsectionpart.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CommonReportsubsectionpart.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CommonReportsubsectionpart` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayaObjReportrow.size(); i++) {
        CommonReportrow.validateJsonObject(jsonArrayaObjReportrow.get(i).getAsJsonObject());
      };
  }
