...
public class ScimGroupMember {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("value", "display", "type", "$ref"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ScimGroupMember.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ScimGroupMember is not found in the empty JSON string", ScimGroupMember.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ScimGroupMember.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ScimGroupMember` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
