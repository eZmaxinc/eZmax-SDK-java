...
public class EzsigntemplateCopyV1Request {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("a_fkiEzsignfoldertypeID", "bCopyCompany", "bCopyUser"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsigntemplateCopyV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in EzsigntemplateCopyV1Request is not found in the empty JSON string", EzsigntemplateCopyV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsigntemplateCopyV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsigntemplateCopyV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("a_fkiEzsignfoldertypeID") != null && !jsonObj.get("a_fkiEzsignfoldertypeID").isJsonNull() && !jsonObj.get("a_fkiEzsignfoldertypeID").isJsonArray()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `a_fkiEzsignfoldertypeID` to be an array in the JSON string but got `%s`", jsonObj.get("a_fkiEzsignfoldertypeID").toString()));
      }
  }
