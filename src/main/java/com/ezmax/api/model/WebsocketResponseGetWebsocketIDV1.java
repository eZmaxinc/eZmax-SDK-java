  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!WebsocketResponseGetWebsocketIDV1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in WebsocketResponseGetWebsocketIDV1 is not found in the empty JSON string", WebsocketResponseGetWebsocketIDV1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WebsocketResponseGetWebsocketIDV1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `WebsocketResponseGetWebsocketIDV1` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
