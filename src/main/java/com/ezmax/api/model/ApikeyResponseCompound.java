  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApikeyResponseCompound.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ApikeyResponseCompound is not found in the empty JSON string", ApikeyResponseCompound.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApikeyResponseCompound.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ApikeyResponseCompound` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("sApikeyApikey") != null && !jsonObj.get("sApikeyApikey").isJsonNull()) && !jsonObj.get("sApikeyApikey").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `sApikeyApikey` to be a primitive type in the JSON string but got `%s`", jsonObj.get("sApikeyApikey").toString()));
      }
