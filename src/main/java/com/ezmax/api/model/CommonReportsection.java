...
public class CommonReportsection {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CommonReportsection
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CommonReportsection.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CommonReportsection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CommonReportsection` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayaObjReportsubsection.size(); i++) {
        CommonReportsubsection.validateJsonObject(jsonArrayaObjReportsubsection.get(i).getAsJsonObject());
      };
      // ensure the json data is an array
      ...
      for (int i = 0; i < jsonArrayaObjReportcolumn.size(); i++) {
        CommonReportcolumn.validateJsonObject(jsonArrayaObjReportcolumn.get(i).getAsJsonObject());
      };
  }
