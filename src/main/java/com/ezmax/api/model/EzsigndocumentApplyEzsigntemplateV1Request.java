  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsigndocumentApplyEzsigntemplateV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsigndocumentApplyEzsigntemplateV1Request is not found in the empty JSON string", EzsigndocumentApplyEzsigntemplateV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsigndocumentApplyEzsigntemplateV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsigndocumentApplyEzsigntemplateV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
