  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EzsignfoldersignerassociationCreateEmbeddedUrlV1Request.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in EzsignfoldersignerassociationCreateEmbeddedUrlV1Request is not found in the empty JSON string", EzsignfoldersignerassociationCreateEmbeddedUrlV1Request.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EzsignfoldersignerassociationCreateEmbeddedUrlV1Request.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `EzsignfoldersignerassociationCreateEmbeddedUrlV1Request` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("sIframedomain") != null && !jsonObj.get("sIframedomain").isJsonNull()) && !jsonObj.get("sIframedomain").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `sIframedomain` to be a primitive type in the JSON string but got `%s`", jsonObj.get("sIframedomain").toString()));
      }
  }
